...
all: $(VOFILES) $(CMOFILES) $(CMXSFILES) src/counting_plugin.cmxs\
  src/counting_plugin.cma
spec: $(VIFILES)
...
install:
	...
	(cd theories; for i in $(VOFILES1); do \
	 install -d `dirname $(COQLIB)/user-contrib/Counting/$$i`; \
	 install $$i $(COQLIB)/user-contrib/Counting/$$i; \
	 done)
...
clean:
	...
	- rm -rf html
	- rm -f src/counting_plugin.cmxs
	- rm -f src/counting_plugin.cma
